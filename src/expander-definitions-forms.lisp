...
(define-expander-for
  ...
  (let* ((fn-name (second form))
    ...
    (if (and
        ...
        (symbolp fn-name)
        (listp (third form))
        (lambda-list-has-typed-p (third form))) (expand-typed-defun-or-lambda
        'defun
        fn-name
        (third form)
        (if docstring (cons docstring body)
          body))
      (list*
        'defun
        fn-name
...
(define-expander-for
  ...
  (let ((body (%strip-docstring (cddr form))))
    (if (and
        (>= (length form) 3)
        (listp (second form))
        (lambda-list-has-typed-p (second form))) (expand-typed-defun-or-lambda 'lambda nil (second form) body)
      (list*
        'lambda
...
(define-expander-for progn (form) (expand-progn-with-eager-defmacro (cdr form)))
    ...
    (cond
      ((and
        ...
        (let* ((lambda-list (third form))
               (body (cdddr form))
               (expansion
              (if (= (length body) 1) (first body)
                `(progn
                  ,@body))))
          (if (null lambda-list) (let ((result (funcall *macro-eval-fn* expansion)))
