...
(defsuite
  ...
  :description
  "Tests for macros-stdlib.lisp: host bridges and file/IO helpers"
  :parent
  cl-cc-serial-suite)
...
(deftest load-time-value-is-memoized-during-expansion "LOAD-TIME-VALUE evaluates identical forms once per compiler session." (let ((*load-time-value-hit* 0)) (let* ((form '(load-time-value (progn (incf *load-time-value-hit*) *load-time-value-hit*))) (first (our-macroexpand-1 form))) (clrhash cl-cc/expand:*macroexpand-step-cache*) (let ((second (our-macroexpand-1 form))) (assert-eq 'quote (car first)) (assert-eq 'quote (car second)) (assert-= 1 (second first)) (assert-= 1 (second second)) (assert-= 1 *load-time-value-hit*)))))
