...
(defsuite
  expander-definitions-forms-suite
  :description
  "Definition-form expander unit tests"
  :parent
  cl-cc-unit-suite)
...
(deftest
  expander-lambda-expands-typed-params
...
(deftest
  expander-defclass-and-deftype-expands
  "defclass registers slots and deftype evaluates aliases through the configured evaluator."
    ...
    (let ((class-result
          ...
          (cl-cc/expand:compiler-macroexpand-all
            (quote
              (deftype sample-type ()
                (quote integer))))))
      (assert-eq (quote defclass) (car class-result))
      (assert-eq (quote quote) (car type-result))
      (assert-eq (quote sample-type) (second type-result)))))
