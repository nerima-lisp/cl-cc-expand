...
(defun ds-assert-deriving-registers (typeclass-name)
  ...
  (cl-cc/expand:with-fresh-defstruct-registries
    (let ((cl-cc/type::*typeclass-registry* (make-hash-table :test #'eq))
      ...
      (eval
        (ds-expand
          `(defstruct (,name (:deriving eq show ord)) x
            y)))
      (assert-true (cl-cc/type::has-typeclass-instance-p typeclass-name name)))))

;;; ─── Basic struct ─────────────────────────────────────────────────────────
(deftest
  ds-basic-expansion-produces-defclass
...
(deftest
  ds-basic-expansion-generates-constructor
...
(deftest
  ds-basic-expansion-generates-predicate
...
(deftest
  ds-boa-constructor-uses-positional-params
  ":constructor with BOA lambda list generates positional params (no &key)."
  (cl-cc/expand:with-fresh-defstruct-registries
    (let* ((exp
          (ds-expand
            '(defstruct (point (:constructor make-pt (x y))) x
              y)))
           (forms (ds-progn-forms exp))
           (ctor (second forms))
           (params (third ctor)))
...
;;; ─── Docstring filtering ──────────────────────────────────────────────────
(deftest
  ds-docstring-ignored
  "Docstring in slot list is filtered out."
  (cl-cc/expand:with-fresh-defstruct-registries
    (let* ((exp
          (ds-expand
            '(defstruct point "A 2D point."
              x
              y)))
           (defclass-form (second exp))
           (slot-specs (fourth defclass-form)))
...
(deftest
  ds-read-only-accessor-not-registered-for-setf
  "Read-only defstruct accessor is excluded from writable accessor map."
  (cl-cc/expand:with-fresh-defstruct-registries
    (let* ((exp
          (ds-expand
            '(defstruct packet (id 0 :read-only t)
              payload)))
      ...
      (assert-true
        (gethash id-accessor cl-cc/expand:*defstruct-read-only-accessor-map*))
      (assert-true (gethash payload-accessor cl-cc/expand:*accessor-slot-map*)))))

(deftest
  ds-read-only-accessor-setf-signals-error
  ...
  (cl-cc/expand:with-fresh-defstruct-registries
    ...
    (assert-signals
      ...
      (cl-cc/expand:compiler-macroexpand-all
        '(setf (cl-cc/expand::packet-id p) 10)))))

(deftest
  ds-empty-struct-has-zero-slots
...
(deftest-each
  ds-extract-boa-parts-normal-only
...
(deftest
  ds-extract-boa-parts-splits-aux-bindings
...
(deftest
  ds-extract-boa-parts-promotes-bare-aux-symbol
...
(defun %ds-tree-member (item tree)
  ...
  (cond
    ...
    ((eq tree item) t)
    ((atom tree) nil)
    (t (or (%ds-tree-member item (car tree)) (%ds-tree-member item (cdr tree))))))

(deftest-each
  ds-type-typed-constructor-cases
  "(:type list/vector) defstruct: constructor body uses the appropriate collection constructor."
  :cases
  (("list"
      '(defstruct (point (:type list)) x
        y)
      'list)
    ("vector"
      '(defstruct (seg (:type vector)) a
        b)
      'vector))
  (form expected-ctor)
  (cl-cc/expand:with-fresh-defstruct-registries
...
(deftest
  ds-type-list-predicate-checks-listp
  "(:type list) defstruct: predicate tests with LISTP."
  (cl-cc/expand:with-fresh-defstruct-registries
    (let* ((exp
          (ds-expand
            '(defstruct (rect (:type list) (:predicate rect-list-p)) width
              height)))
           (forms (ds-progn-forms exp))
           (pred
          (find-if
            (lambda (f)
              (and (listp f) (eq (first f) 'defun) (eq (second f) 'rect-list-p)))
            forms)))
...
(deftest
  ds-basic-expansion-generates-copier
  ...
  (cl-cc/expand:with-fresh-defstruct-registries
    (let* ((exp
          ...
          (find-if
            (lambda (f)
              (and (listp f) (eq (first f) 'defun) (eq (second f) (intern "COPY-POINT"))))
            forms)))
      (assert-true (not (null copier))))))

(deftest
  ds-copier-nil-suppresses-copier
  "(:copier nil) suppresses copier generation."
  (cl-cc/expand:with-fresh-defstruct-registries
    (let* ((exp
          (ds-expand
            '(defstruct (point (:copier nil)) x
              y)))
           (forms (ds-progn-forms exp))
           (copier
          (find-if
...
(deftest
  ds-copier-custom-name
  "(:copier custom-name) uses the custom name instead of COPY-NAME."
  (cl-cc/expand:with-fresh-defstruct-registries
    (let* ((exp
          (ds-expand
            '(defstruct (point (:copier clone-point)) x
              y)))
           (forms (ds-progn-forms exp))
           (copier
          (find-if
            (lambda (f)
              (and (listp f) (eq (first f) 'defun) (eq (second f) 'clone-point)))
            forms)))
      (assert-true (not (null copier))))))

(deftest
  ds-print-object-generates-print-object-method
  "(:print-object fn) emits a PRINT-OBJECT method calling FN with object and stream."
  (cl-cc/expand:with-fresh-defstruct-registries
    (let* ((exp
          (ds-expand
            '(defstruct (point (:print-object print-point)) x
              y)))
           (forms (ds-progn-forms exp))
           (method
          (find-if
            (lambda (f)
              (and (listp f) (eq (first f) 'defmethod) (eq (second f) 'print-object)))
            forms))
           (body (fourth method)))
...
(deftest
  ds-print-function-generates-print-object-method
  "(:print-function fn) emits a PRINT-OBJECT bridge calling FN with object, stream, depth."
  (cl-cc/expand:with-fresh-defstruct-registries
    (let* ((exp
          (ds-expand
            '(defstruct (point (:print-function print-point)) x
              y)))
           (forms (ds-progn-forms exp))
           (method
          (find-if
...
(deftest
  ds-build-constructor-keyword-form
  "%defstruct-build-constructor with no BOA args builds (defun ctor (&key ...) body)."
  (let* ((slots '((x 0) (y 1)))
         (result
        (cl-cc/expand::%defstruct-build-constructor
          'make-pt
          nil
          slots
          (lambda (svs)
